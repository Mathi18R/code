...
class GRU_plain(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, num_layers, has_input=True, has_output=False, output_size=None):
        ...
        if has_input:
            print("types: " + str(input_size) + "\t" + str(embedding_size))
            self.my_input_layer = nn.Linear(input_size, embedding_size)
        ...
        for name, param in self.rnn.named_parameters():
            if 'bias' in name:
                nn.init.constant_(param, 0.25) # constant -> constant_
            elif 'weight' in name:
                nn.init.xavier_uniform_(param,gain=nn.init.calculate_gain('sigmoid')) # xavier_uniform -> xavier_uniform_
        for m in self.modules():
            if isinstance(m, nn.Linear):
                m.weight.data = init.xavier_uniform_(m.weight.data, gain=nn.init.calculate_gain('relu')) # xavier_uniform -> xavier_uniform_

    def init_hidden(self, batch_size):
        return Variable(torch.zeros(self.num_layers, batch_size, self.hidden_size))#.cuda() -> removed for conversion

    def forward(self, input_raw, pack=False, input_len=None):
        if self.has_input:
            input = self.my_input_layer(input_raw)
            input = self.relu(input)
        else:
            input = input_raw
        if pack:
        ...
        if pack:
            output_raw = pad_packed_sequence(output_raw, batch_first=True)[0]
        if self.has_output:
            output_raw = self.my_output_layer(output_raw)
        # return hidden state at each time step
        return output_raw
