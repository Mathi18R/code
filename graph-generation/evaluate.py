...
def eval_single_list_graphs(graphs_pred, graphs_ref):
    ''' Evaluate a list of graphs by comparing with graphs in directory dir_input.
    Args:
        dir_input: directory where ground truth graph list is stored
        dataset_name: name of the dataset (ground truth)
    '''
    graph_ref_len = len(graphs_ref)
    graphs_ref = graphs_ref[int(0.8 * graph_ref_len):] # test on a hold out test set
    #mmd_degree = eval.stats.degree_stats(graphs_ref, graphs_pred)
    #mmd_clustering = eval.stats.clustering_stats(graphs_ref, graphs_pred)
    try:
        mmd_4orbits = eval.stats.orbit_stats_all(graphs_ref, graphs_pred)
...
if __name__ == '__main__':
    graphs_pred = load_graph_list('input/graphs_pred090.dat')
    graphs_ref = load_graph_list('input/graphs_ref090.dat')
    eval_single_list_graphs(graphs_pred, graphs_ref)
