...
def train_vae_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        y_pred = torch.sigmoid(y_pred) #F -> torch
        # clean
        ...
        loss_sum += loss.data #loss.data[0]->loss.data
...
def test_vae_epoch(epoch, args, rnn, output, test_batch_size=16, save_histogram=False, sample_time = 1):
    ...
    for i in range(max_num_node):
        ...
        y_pred[:, i:i + 1, :] = torch.sigmoid(y_pred_step) #F -> torch
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        if epoch % args.epochs_log==0 and batch_idx==0: # only output first batch's statistics
            print('Epoch: {}/{}, train loss: {:.6f}, graph type: {}, num_layer: {}, hidden: {}'.format(
                epoch, args.epochs,loss.data, args.graph_type, args.num_layers, args.hidden_size_rnn)) #loss.data[0]->loss.data

        # logging
        log_value('loss_'+args.fname, loss.data, epoch*args.batch_ratio+batch_idx) #loss.data[0]->loss.data
...
def train_rnn_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        #print("x: " + str(x.size()) + "\th: " + str(h.size())) # test added for debugging in onnx conversion
        # reverse h
        ...
        loss_sum += loss.data*feature_dim#loss.data[0]->loss.data
    return loss_sum/(batch_idx+1)
...
def test_rnn_epoch(epoch, args, rnn, output, test_batch_size=16):
    ...
    for i in range(test_batch_size):
        adj_pred = decode_adj(y_pred_long_data[i].cpu().numpy())
        G_pred = get_graph(adj_pred) # get a graph from zero-padded adj #<-dont do get_graph if you want to keep single node islands
        G_pred_list.append(G_pred)
...
def train_rnn_forward_epoch(epoch, args, rnn, output, data_loader):
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        loss_sum += loss.data*feature_dim/y.size(0) #loss.data[0]->loss.data
    return loss_sum/(batch_idx+1)
...
def train(args, dataset_train, rnn, output):
    ...
    while epoch<=args.epochs:
        ...
        print("epoch\t" + str(epoch) + "/" + str(args.epochs) + "\t time:\t" + str(time_end - time_start))
        # test
...
def train_nll(args, dataset_train, dataset_test, rnn, output,graph_validate_len,graph_test_len, max_iter = 1000):
    ...
    epoch = args.load_epoch
    print('model loaded!, epoch: {}'.format(args.load_epoch))
    fname_output = args.nll_save_path + args.note + '_' + args.graph_type + '_' + args.load_epoch  + '.csv'
